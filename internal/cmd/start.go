...
func Start(workspaceRoot, projectRoot string, stateRepo *state.Repository, kataName string, f StartFlags, translator i18n.Translator) error {
	...
	// Override runner if flag is provided
	if f.Runner != "" {
		slog.Info(translator.T("start.log_runner_override"), "old", cfg.Runner.Name, "new", f.Runner)
		cfg.Runner.Name = f.Runner
	}

	runnerName := cfg.Runner.Name
	if runnerName != "" {
		slog.Info(translator.T("start.log_runner_selected"), "runner", runnerName)
		...
		slog.Info("No runner configured for this kata. Relying on existing Taskfile.")
	}
