...
1. Resolving the kata from the local catalog or the embedded catalog
2. Copying the kata scaffold into the project root
3. Copying the configured runner into the project root
4. Applying the first step
5. Initializing the kata state

The kata state is stored in .kata/current_state.json.
...
At the moment, template contents are copied as-is.

Future versions will support rendering using Go’s text/template, with dynamic variables provided by:
