...
When modifying the Kata codebase itself, agents must ensure the following:

- **Formatting**: Always run `go fmt ./...` (or equivalent) after code changes.
- **Testing**: Always run `task test` to ensure no regressions were introduced.
- **Linting**: Always run `task lint` to ensure code style and quality standards are met.
- **i18n**: If adding or removing keys, always run the i18n codegen and validation tools.

Agents should not consider a task complete until these verification steps pass.

---

## Commit Workflow

When performing commits on the Kata codebase itself, agents must follow this safety and quality protocol:

1. **Verification**: Run `task lint` and ensure all tests pass. Resolve any regressions or linting issues (e.g., unused i18n keys).
2. **Staging**: Stage the relevant changes using `git add`.
3. **User Review**: Present the staged changes to the user using `git diff --staged` and request explicit approval.
4. **Commit**: Only execute `git commit` after the user has approved the changes.
